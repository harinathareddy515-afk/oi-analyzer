...
def pick_levels(d, spot, step, side, n=4):
    ...
    # Rank by evidence, then enforce spacing so three neighbouring strikes
    # don't all represent the same OI wall.
    candidates = candidates.sort_values(
        ["_level_score", oi_col], ascending=[False, False]
    )
    chosen = []
...
def calculate_levels(df, spot, step):
    ...
    def primary(rows):
        if not rows:
            return None
        return max(rows, key=lambda r: float(r.get("_level_score", 0.0)))
