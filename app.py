...
INDEXES = {
    "NIFTY 50": {"key": "NSE_INDEX|Nifty 50", "step": 50, "strike_window": 14, "exchange": "NSE", "symbol": "NIFTY", "mode": "full"},
    "BANK NIFTY": {"key": "NSE_INDEX|Nifty Bank", "step": 100, "strike_window": 14, "exchange": "NSE", "symbol": "BANKNIFTY", "mode": "full"},
}

INDEX_FALLBACKS = {
    "NIFTY FIN SERVICE": {"key": "NSE_INDEX|Nifty Fin Service", "step": 50, "symbol": "FINNIFTY"},
    "NIFTY MIDCAP SELECT": {"key": "NSE_INDEX|Nifty Midcap Select", "step": 25, "symbol": "MIDCPNIFTY"},
    "NIFTY NEXT 50": {"key": "NSE_INDEX|Nifty Next 50", "step": 50, "symbol": "NIFTYNXT50"},
    "SENSEX": {"key": "BSE_INDEX|SENSEX", "step": 100, "exchange": "BSE", "symbol": "SENSEX"},
    "BANKEX": {"key": "BSE_INDEX|BANKEX", "step": 100, "exchange": "BSE", "symbol": "BANKEX"},
}

def _index_label(row):
    name = str(row.get("name") or row.get("trading_symbol") or "").strip()
    exchange = str(row.get("exchange") or row.get("segment", "")).replace("_INDEX", "")
    return name, exchange

@st.cache_data(ttl=86400, show_spinner=False)
def discover_index_catalog():
    """Load Upstox's daily instrument universe and expose available indices."""
    catalog = dict(INDEX_FALLBACKS)
    urls = [
        "https://assets.upstox.com/market-quote/instruments/exchange/complete.json.gz",
        "https://assets.upstox.com/market-quote/instruments/global.json.gz",
    ]
    for url in urls:
        try:
            ...
            for row in data:
                ...
                if not ((seg in ("NSE_INDEX", "BSE_INDEX") and typ == "INDEX") or seg == "GLOBAL_INDEX"):
                    continue
                name, exchange = _index_label(row)
                key = row.get("instrument_key")
                if not name or not key:
                    continue
                display = f"{name} [{exchange}]" if seg != "GLOBAL_INDEX" else f"{name} [GLOBAL]"
                catalog[display] = {
                    "key": key, "step": 50, "strike_window": 14,
                    "exchange": exchange or ("GLOBAL" if seg == "GLOBAL_INDEX" else ""),
                    "symbol": row.get("trading_symbol") or name,
                    "mode": "context" if seg == "GLOBAL_INDEX" else "auto",
                    "segment": seg,
                }
        except Exception:
            continue
    catalog["NIFTY 50"] = INDEXES["NIFTY 50"]
    catalog["BANK NIFTY"] = INDEXES["BANK NIFTY"]
    return dict(sorted(catalog.items(), key=lambda kv: (0 if kv[0] in ("NIFTY 50","BANK NIFTY") else 1, kv[0])))
...
def oi_heatmap(df, spot, step, levels=None):
    """Trader-first OI battlefield: fixed strike column, compact bars, readable ΔOI and anchors."""
    levels = levels or {}
    ...
    ce_wall = float(x.loc[ce.idxmax(), "strike"]) if len(x) else None
    pe_wall = float(x.loc[pe.idxmax(), "strike"]) if len(x) else None
    net = float(pe.sum()-ce.sum())
    def pct(v, mx): return max(5.0, min(100.0, float(v)/mx*100.0))
    ...
    def tag(strike):
        ...
        if ce_wall is not None and abs(strike-ce_wall) <= step*.1: tags.append(("CALL WALL","wallce"))
        if pe_wall is not None and abs(strike-pe_wall) <= step*.1: tags.append(("PUT WALL","wallpe"))
    ...
    for _,r in x.iterrows():
        ...
        cstate='build' if cdelta>0 else 'unwind' if cdelta<0 else 'flat'
        pstate='build' if pdelta>0 else 'unwind' if pdelta<0 else 'flat'
        rows.append(f"""
        <div class="hm-row{rowcls}">
          <div class="hm-side ce"><div class="bar cebar" style="width:{pct(cv,cmax):.1f}%"></div><div class="hm-content"><div class="val">{fmt(cv)}</div><div class="doi {cstate}">{shift(cdelta)}</div></div></div>
          <div class="strike"><b>{int(strike):,}</b>{tag(strike)}</div>
          <div class="hm-side pe"><div class="bar pebar" style="width:{pct(pv,pmax):.1f}%"></div><div class="hm-content"><div class="val">{fmt(pv)}</div><div class="doi {pstate}">{shift(pdelta)}</div></div></div>
        </div>""")
    bias = "PUT-heavy" if net > 0 else "CALL-heavy" if net < 0 else "BALANCED"
    ...
    *{{box-sizing:border-box}}body{{margin:0;font-family:Inter,ui-sans-serif,system-ui,-apple-system,Segoe UI,sans-serif;background:#fff;color:#172033}}
    .summary{{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:10px}}.sum{{border:1px solid #e6eaf0;border-radius:12px;padding:9px 12px;background:#fbfcfe}}.sum .k{{font-size:9px;text-transform:uppercase;letter-spacing:.08em;color:#64748b;font-weight:800}}.sum .v{{font-size:15px;font-weight:850;margin-top:3px}}.sum.ce .v{{color:#b42318}}.sum.pe .v{{color:#047857}}.sum.bias .v{{color:#334155}}
    .card{{border:1px solid #e1e6ee;border-radius:16px;overflow:hidden;background:#fff;box-shadow:0 5px 20px rgba(15,23,42,.045)}}.head{{display:grid;grid-template-columns:1fr 112px 1fr;background:#f7f9fc;border-bottom:1px solid #e1e6ee;font-size:10px;font-weight:850;letter-spacing:.08em;text-transform:uppercase;color:#64748b}}.head div{{padding:10px 14px}}.ceh{{text-align:right;color:#b42318}}.peh{{color:#047857}}
    .hm-row{{display:grid;grid-template-columns:1fr 112px 1fr;min-height:48px;border-bottom:1px solid #eef1f5}}.hm-row:last-child{{border-bottom:0}}.hm-row.focus{{background:#f8fbff;box-shadow:inset 0 2px 0 #64748b,inset 0 -2px 0 #64748b}}
    .hm-side{{position:relative;display:flex;align-items:center;padding:5px 12px;overflow:hidden}}.hm-side.ce{{justify-content:flex-end;text-align:right;border-right:1px solid #edf0f4}}.hm-side.pe{{border-left:1px solid #edf0f4}}
    .bar{{position:absolute;top:5px;bottom:5px;border-radius:8px;z-index:0;opacity:.9}}.cebar{{right:7px;background:linear-gradient(90deg,rgba(220,38,38,.06),rgba(220,38,38,.62))}}.pebar{{left:7px;background:linear-gradient(90deg,rgba(5,150,105,.62),rgba(5,150,105,.06))}}
    .hm-content{{position:relative;z-index:1;display:flex;align-items:center;gap:9px}}.ce .hm-content{{justify-content:flex-end}}.val{{font-size:13px;font-weight:850;min-width:58px}}.doi{{font-size:10px;font-weight:800;min-width:54px}}.build{{color:#15803d}}.unwind{{color:#b91c1c}}.flat{{color:#94a3b8}}
    .strike{{display:flex;flex-direction:column;align-items:center;justify-content:center;background:#fff;border-left:1px solid #e8ebf0;border-right:1px solid #e8ebf0;gap:3px}}.strike b{{font-size:13px}}.tag{{font-size:7px;font-weight:900;padding:2px 5px;border-radius:999px;letter-spacing:.04em;white-space:nowrap}}.atm{{background:#e2e8f0;color:#334155}}.eos{{background:#dcfce7;color:#166534}}.eor{{background:#fee2e2;color:#b91c1c}}.wallce{{background:#ffe4e6;color:#9f1239}}.wallpe{{background:#d1fae5;color:#065f46}}
    .legend{{display:flex;gap:14px;flex-wrap:wrap;padding:9px 12px;background:#fbfcfe;border-top:1px solid #eef1f5;font-size:9px;color:#64748b}}.legend b{{color:#334155}}@media(max-width:760px){{.summary{{grid-template-columns:1fr 1fr}}.head,.hm-row{{grid-template-columns:1fr 88px 1fr}}.val{{font-size:12px}}.doi{{font-size:9px;min-width:42px}}}}
    </style></head><body><div class="summary"><div class="sum ce"><div class="k">Strongest call wall</div><div class="v">{int(ce_wall):,} · {fmt(ce.max()) if len(ce) else '—'}</div></div><div class="sum pe"><div class="k">Strongest put wall</div><div class="v">{int(pe_wall):,} · {fmt(pe.max()) if len(pe) else '—'}</div></div><div class="sum"><div class="k">ATM</div><div class="v">{spot:,.0f}</div></div><div class="sum bias"><div class="k">Chain balance</div><div class="v">{bias}</div></div></div><div class="card"><div class="head"><div class="ceh">CALL OI / ΔOI</div><div style="text-align:center">STRIKE</div><div class="peh">PUT OI / ΔOI</div></div>{''.join(rows)}<div class="legend"><span>🔴 CE resistance</span><span>🟢 PE support</span><span><b>↑</b> OI building</span><span><b>↓</b> OI unwinding</span><span><b>ATM</b> spot area</span><span><b>EOS/EOR</b> decision anchors</span></div></div></body></html>"""
    components.html(html, height=108+len(rows)*48, scrolling=False)


def infer_strike_step(df, fallback=50):
    try:
        vals = pd.to_numeric(df["strike"], errors="coerce").dropna().sort_values().unique()
        diffs = np.diff(vals)
        diffs = diffs[diffs > 0]
        if len(diffs):
        ...
        pass
    return float(fallback or 50)


def render_index_basic(name, token, cfg):
    """Readable index-only view for instruments without an option chain."""
    try:
        spot, cp = underlying_ltp(token, cfg["key"])
    except Exception as e:
        st.error(f"{name}: market quote unavailable — {e}")
        return
    move = (spot/cp-1)*100 if cp else 0.0
    try:
    ...
    direction = "UP" if move > 0.05 else "DOWN" if move < -0.05 else "FLAT"
    icon = "🟢" if direction == "UP" else "🔴" if direction == "DOWN" else "🟡"
    st.markdown(f"## {name} <span class='index-mode-pill'>INDEX MODE</span>", unsafe_allow_html=True)
    st.markdown(f"""
    <div class="basic-hero">
      <div><div class="eyebrow">LIVE INDEX</div><div class="basic-spot">{spot:,.2f}</div><div class="basic-move">{icon} {move:+.2f}% today</div></div>
      <div class="basic-status"><div class="eyebrow">DATA CAPABILITY</div><strong>Index price & structure</strong><span>Options/OI analytics are not available for this instrument.</span></div>
    </div>
    """, unsafe_allow_html=True)
    a,b,c,d,e,f = st.columns(6)
    a.metric("5m", f"{market.get('ret5',0):+.2f}%")
    b.metric("15m", f"{market.get('ret15',0):+.2f}%")
    c.metric("VWAP", f"{vwap:,.2f}" if vwap else "—", f"{spot-vwap:+.1f}" if vwap else None)
    d.metric("Opening range", f"{market.get('or_low',0):,.0f}–{market.get('or_high',0):,.0f}" if market.get('or_high') else "—")
    e.metric("Session H/L", f"{market.get('session_high',0):,.0f} / {market.get('session_low',0):,.0f}" if market.get('session_high') else "—")
    f.metric("Previous close", f"{prev.get('prev_close',0):,.2f}" if prev else "—")
    st.markdown("### Price structure")
    if vwap:
        st.info("🟢 Price is above VWAP." if spot > vwap else "🔴 Price is below VWAP.")
    if prev:
        p1,p2,p3 = st.columns(3)
        p1.metric("Prev high", f"{prev.get('prev_high',0):,.2f}")
        p2.metric("Prev low", f"{prev.get('prev_low',0):,.2f}")
        p3.metric("Prev close", f"{prev.get('prev_close',0):,.2f}")
    st.warning("Options/OI battlefield, EOS/EOR and option-derived setup analytics are unavailable for this index. The terminal automatically switched to index-only mode instead of inventing data.")
...
def render_contributors(token, name):
    ...
    if uploaded_weights is not None:
        ...
        st.info("Index-impact stock flow is available for NIFTY 50 and BANK NIFTY by default. Upload a current index-weight CSV to enable this view for another index.")
        return
...
def search_futures(token, name):
    ...
    q = str(INDEXES.get(name, {}).get("symbol") or name).replace(" [NSE]", "").replace(" [BSE]", "")[:40]
    try:
...
def render_index_v3(name, token):
    ...
    try:
        ...
        if not expiry:
            render_index_basic(name, token, cfg)
            return
        chain = option_chain(token, cfg["key"], expiry)
        raw_df = build_option_df(chain)
        if raw_df.empty:
            render_index_basic(name, token, cfg)
            return
        step = infer_strike_step(raw_df, cfg.get("step", 50))
        cfg = dict(cfg); cfg["step"] = step
        INDEXES[name] = cfg
        df = enrich_tick(raw_df, name)
    except Exception as e:
        try:
            render_index_basic(name, token, cfg)
        except Exception:
...
def inject_trader_css():
    st.markdown("""
    <style>
    :root{--ink:#0f172a;--muted:#64748b;--line:#e5eaf1;--bg:#f4f7fb}
    .block-container{max-width:1540px;padding-top:.75rem;padding-bottom:3.5rem}.stApp{background:linear-gradient(180deg,#f7f9fc 0%,#f2f5f9 100%)}
    h1,h2,h3{color:var(--ink);letter-spacing:-.028em}h2{margin-top:1.25rem!important}
    [data-testid="stMetric"]{background:rgba(255,255,255,.96);border:1px solid var(--line);border-radius:14px;padding:.62rem .82rem;box-shadow:0 4px 18px rgba(15,23,42,.035)}
    [data-testid="stMetricLabel"]{font-size:.72rem!important;color:#64748b!important}[data-testid="stMetricValue"]{font-size:1.25rem!important;color:#0f172a!important}
    .trader-shell{background:linear-gradient(135deg,#fff 0%,#f8fbff 100%);border:1px solid #dfe6ef;border-radius:20px;padding:16px 18px;box-shadow:0 10px 32px rgba(15,23,42,.055);margin-bottom:12px}
    .decision{display:grid;grid-template-columns:1.15fr .8fr .8fr 1fr;gap:9px}.decision .box{border:1px solid #e7ebf1;border-radius:14px;padding:12px 14px;background:rgba(255,255,255,.86);min-height:82px}
    .decision .label{font-size:9px;text-transform:uppercase;letter-spacing:.1em;color:#64748b;font-weight:850}.decision .big{font-size:23px;font-weight:900;line-height:1.12;color:#0f172a;margin-top:4px}.decision .sub{font-size:10px;color:#64748b;margin-top:5px}
    .bull{border-left:4px solid #10b981!important}.bear{border-left:4px solid #ef4444!important}.range{border-left:4px solid #f59e0b!important}
    .mini-grid{display:grid;grid-template-columns:repeat(8,1fr);gap:7px;margin-top:8px}.mini{background:rgba(255,255,255,.92);border:1px solid #e6eaf0;border-radius:11px;padding:8px 10px;min-height:52px}.mini .k{font-size:9px;color:#64748b;text-transform:uppercase;font-weight:850;letter-spacing:.05em}.mini .v{font-size:13px;font-weight:850;color:#172033;margin-top:3px}
    .pulse-card{min-height:86px;background:#fff;border:1px solid #e7ebf0;border-radius:14px;padding:11px 12px;box-shadow:0 3px 14px rgba(15,23,42,.035)}.pulse-top{font-size:11px;color:#475569}.pulse-main{font-size:14px;font-weight:850;margin-top:5px}.pulse-sub{font-size:10px;color:#64748b;margin-top:3px}
    .level-stack{display:flex;flex-direction:column;gap:5px}.level-stack .lvl{display:grid;grid-template-columns:70px 1fr 1fr;align-items:center;padding:9px 12px;border-radius:11px}.level-stack .lvl b{font-size:17px;text-align:center}.level-stack small{text-align:right;color:#64748b}.redlvl{background:#fff1f2;color:#9f1239}.greenlvl{background:#ecfdf5;color:#047857}.spotlvl{background:#eff6ff;color:#1d4ed8;border:1px solid #bfdbfe}.ext{opacity:.72}
    div[data-testid="stExpander"]{border:1px solid #e0e6ee!important;border-radius:13px!important;background:rgba(255,255,255,.82)}div[data-testid="stExpander"] summary{font-weight:750;color:#1e293b}
    .index-mode-pill{display:inline-block;font-size:9px;letter-spacing:.08em;text-transform:uppercase;background:#e2e8f0;color:#475569;border-radius:999px;padding:4px 8px;vertical-align:middle}
    .basic-hero{display:flex;justify-content:space-between;gap:18px;align-items:stretch;background:linear-gradient(135deg,#fff,#f8fbff);border:1px solid #dfe6ef;border-radius:18px;padding:18px 20px;box-shadow:0 8px 28px rgba(15,23,42,.045);margin-bottom:12px}.eyebrow{font-size:9px;text-transform:uppercase;letter-spacing:.1em;font-weight:850;color:#64748b}.basic-spot{font-size:34px;font-weight:900;letter-spacing:-.04em;color:#0f172a}.basic-move{font-size:13px;font-weight:800;margin-top:3px}.basic-status{min-width:300px;border:1px solid #e5eaf1;border-radius:14px;padding:12px 14px;background:#fff;display:flex;flex-direction:column;justify-content:center}.basic-status strong{font-size:15px;margin:4px 0}.basic-status span{font-size:11px;color:#64748b;line-height:1.4}
    .stTabs [data-baseweb="tab-list"]{gap:6px;background:#eef2f7;border-radius:12px;padding:4px}.stTabs [data-baseweb="tab"]{border-radius:9px;padding:7px 13px;font-weight:750}.stTabs [aria-selected="true"]{background:#fff;box-shadow:0 2px 8px rgba(15,23,42,.06)}
    @media(max-width:1100px){.mini-grid{grid-template-columns:repeat(4,1fr)}.decision{grid-template-columns:1fr 1fr}.basic-status{min-width:0}}@media(max-width:700px){.mini-grid{grid-template-columns:repeat(2,1fr)}.basic-hero{flex-direction:column}.basic-spot{font-size:28px}}
    </style>
    """, unsafe_allow_html=True)
...
def main():
    ...
    global INDEXES
    try:
        INDEXES.update(discover_index_catalog())
    except Exception:
        pass

    st.title("⚡ OI Pulse Pro")
    st.caption("Intraday index intelligence • OI battlefield • levels • futures • regime • index drivers • research")
    index_names=list(INDEXES.keys())
    default_idx=index_names.index("NIFTY 50") if "NIFTY 50" in index_names else 0
    selected=st.selectbox("Trade / monitor index", index_names, index=default_idx, key="selected_index", help="Domestic indices with option chains receive the full OI/levels engine. Other supported indices automatically switch to index-only mode.")
    cfg=INDEXES[selected]
    mode="FULL OI / OPTIONS" if cfg.get("mode") in ("full","auto") else "INDEX / CONTEXT"
    st.markdown(f"<div class='trader-shell' style='padding:9px 13px'><b>{selected}</b> <span style='color:#64748b;font-size:11px'>• {mode} • Upstox instrument: {cfg.get('key')}</span></div>", unsafe_allow_html=True)

    @st.fragment(run_every=f"{refresh}s")
    def live():
        render_cross_index(token)
        render_index_v3(selected,token)
        with st.expander("🧪 Signal Validation / Research", expanded=False):
            validation_panel()
        with st.expander("🧬 Feature Lab / Evidence Gates", expanded=False):
            feature_gate_panel()
    live()
