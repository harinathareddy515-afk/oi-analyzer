...
def render_upstox_tv_chart(candles, spot, levels, name):
    """
    TradingView-style candlestick chart using the TradingView Lightweight Charts
    library, with candles sourced from Upstox V3 and our calculated OI levels.
    """
    import streamlit.components.v1 as components
    ...
    _chart_status = candles.attrs.get("chart_status", "UPSTOX CANDLES")
    _session_date = candles.attrs.get("session_date", "")
    if _chart_status.startswith("LAST SESSION"):
        st.info(f"🕒 Market closed / non-trading day — showing the latest completed session ({_session_date}). Live candles will resume automatically when the market opens.")
    ...
    lvl=_json.dumps(line_levels)
    safe_name=_json.dumps(str(name))
    ...
    <style>
      html,body{{margin:0;padding:0;background:#ffffff;font-family:Inter,Arial,sans-serif;overflow:hidden}}
      #wrap{{width:100%;height:610px;position:relative;border:1px solid #e5e7eb;border-radius:14px;overflow:hidden}}
      #chart{{width:100%;height:100%}}
      #legend{{position:absolute;left:14px;top:12px;z-index:5;background:rgba(255,255,255,.92);
        border:1px solid #e5e7eb;border-radius:9px;padding:8px 11px;font-size:12px;color:#334155;
        box-shadow:0 2px 8px rgba(15,23,42,.08)}}
      .up{{color:#047857;font-weight:700}} .down{{color:#b91c1c;font-weight:700}}
    </style></head>
    <body>
      <div id="wrap"><div id="chart"></div><div id="legend">{safe_name} • Upstox candles • OI levels</div></div>
      <script>
      (function(){{
        const data={payload};
        const levels={lvl};
        ...
        function start(){{
          if(!window.LightweightCharts){{ setTimeout(start,250); return; }}
          const chart=LightweightCharts.createChart(el,{{
            width:el.clientWidth,height:610,
            layout:{{background:{{type:'solid',color:'#ffffff'}},textColor:'#334155',
                     fontFamily:'Inter,Arial,sans-serif'}},
            grid:{{vertLines:{{color:'#f1f5f9'}},horzLines:{{color:'#f1f5f9'}}}},
            crosshair:{{mode:LightweightCharts.CrosshairMode.Normal}},
            rightPriceScale:{{borderColor:'#e2e8f0',scaleMargins:{{top:.08,bottom:.14}}}},
          ...
          cs.setData(data);
          levels.forEach(x=>{{
            const color=x.side==='support'?'#059669':'#dc2626';
            const ls=cs.createPriceLine({{
              price:x.price,color:color,lineWidth:2,
              lineStyle:0,axisLabelVisible:true,title:(x.side==='support'?'S ':'R ')+x.price.toFixed(2)
            }});
            if(Math.abs(x.structural-x.price)>0.01){{
              cs.createPriceLine({{
                price:x.structural,color:color,lineWidth:1,lineStyle:2,
...
def pick_levels(d, spot, step, side, n=4):
    """Pick nearby, meaningful OI levels rather than distant OI extremes."""
    ...
    distance = (src["strike"] - spot).abs()
    primary = src[distance <= 5.0 * step].copy()
    if primary.empty:
        primary = src[distance <= 10.0 * step].copy()
    if primary.empty:
        primary = src.copy()

    dist = (primary["strike"] - spot).abs()
    proximity = np.exp(-dist / max(1.5 * step, 1.0))
    strength = pd.to_numeric(primary[col], errors="coerce").fillna(0).clip(lower=0)
    primary["_level_score"] = strength * (0.35 + 0.65 * proximity)
    primary = primary.sort_values("_level_score", ascending=False)

    nearest_idx = (primary["strike"] - spot).abs().idxmin()
    chosen = [primary.loc[nearest_idx]]
    for _, r in primary.iterrows():
        if all(abs(float(r.strike) - float(q.strike)) >= step * 0.9 for q in chosen):
            chosen.append(r)
        if len(chosen) >= n:
            break

    return sorted(chosen, key=lambda r: float(r.strike))[:n]
...
def calculate_levels(df, spot, step):
    ...
    eos_row = sup[0] if sup else None
    eor_row = res[0] if res else None
    ...
    # Precision companions for the first extension strike. These are useful
    # for traders who want an exact line rather than only the rounded strike.
    eos1_row = df.loc[(df.strike-(eos-step)).abs().idxmin()] if not df.empty else None
    eor1_row = df.loc[(df.strike-(eor+step)).abs().idxmin()] if not df.empty else None
...
def render_index_overview(name, token):
    ...
    st.markdown(f"## {name} <span style='font-size:12px;color:#64748b;font-weight:500'>• index-only analytics • refreshed every few seconds</span>", unsafe_allow_html=True)
    st.markdown(f"""
...
def render_index_v3(name, token):
    ...
    # -----------------------------
    st.markdown("## ⚡ What changed since the last refresh")
    change_rows = []
    ...
    if change_rows:
        ...
        st.caption("No material change detected since the previous refresh.")
...
def main():
    ...
    # Search is useful when a newly listed index is not yet present in the cached BOD file.
    with st.sidebar:
        st.markdown("### 🔎 Find an index")
        search_q = st.text_input("Search NSE/BSE index", placeholder="e.g. NIFTY IT, SENSEX, BANKEX", key="index_search")
        if search_q.strip():
            results = search_index_instruments(search_q.strip())
            if results:
                register_index_configs(results)
                st.caption(f"{len(results)} matching index instrument(s) found.")
            else:
                st.caption("No index match found.")
