    ...
    <body>
      ...
      (function(){{
        ...
        function start(){{
          ...
          // Keep the calculated levels inside the visible price scale.
          // Lightweight Charts autoscaling is primarily candle-driven; a
          // price line can otherwise sit just outside the candle range and
          // appear to have disappeared.  A transparent helper series forces
          // the chart to include every active OI level without adding visual
          // clutter. This works for NIFTY, BANK NIFTY and every dynamically
          // discovered index using the same generic level payload.
          const helperData=[];
          const t0=data.length ? data[0].time : null;
          const t1=data.length ? data[data.length-1].time : null;
          if(t0!==null && t1!==null){{
            levels.forEach(x=>{{
              const prices=[Number(x.price)];
              if(Number.isFinite(Number(x.structural)) && Math.abs(Number(x.structural)-Number(x.price))>0.01)
                prices.push(Number(x.structural));
              prices.forEach((p,i)=>{{
                if(Number.isFinite(p)) helperData.push({{time:t0 + i*60, value:p}});
                if(Number.isFinite(p)) helperData.push({{time:t1, value:p}});
              }});
            }});
          }}
          if(helperData.length){{
            const hs=chart.addLineSeries({{
              ...
              lastValueVisible:false, visible:true
            }});
            hs.setData(helperData.sort((a,b)=>a.time-b.time));
          }}
