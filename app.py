...
def upstox_intraday_candles_df(token, key, minutes=1):
    """Fetch intraday candles from Upstox V3 and normalize them for charting."""
    try:
        d = api_get3(f"/historical-candle/intraday/{quote(key, safe='')}/minutes/{int(minutes)}", token)
        ...
        for c in candles:
            ...
            ts=c[0]
            try:
                # Upstox timestamps are ISO strings in the candle API.
        ...
        return pd.DataFrame(rows).drop_duplicates("time").sort_values("time").tail(390)
    except Exception:
        return pd.DataFrame(columns=["time","open","high","low","close","volume"])
...
def render_upstox_tv_chart(candles, spot, levels, name):
    ...
    if candles is None or getattr(candles, "empty", True):
        st.warning(f"{name}: live candle data is not available yet.")
        return
...
def render_index_v3(name, token):
    ...
    try:
        _tv_candles = upstox_intraday_candles_df(token, cfg["key"], 1)
        st.markdown("### Price Chart — Upstox Live Data")
        render_upstox_tv_chart(_tv_candles, spot, levels, name)
        st.caption("Candles: Upstox V3 • Lines: live OI-derived precision levels • Refresh follows the dashboard.")
    ...
    candidates = [
        ...
        ("EOS−1", float(levels["eos1"]), "support"),
        ("EOR+1", float(levels["eor1"]), "resistance"),
    ]
