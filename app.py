...
def render_index_v3(name, token):
    ...
    # LIVE LEVEL CHART: calculated option-structure levels are plotted on price.
    # This is deliberately guarded so an index without OHLC data continues to work.
    try:
        _chart_ohlc = locals().get("ohlc", locals().get("candles", locals().get("hist", None)))
        if _chart_ohlc is not None and locals().get("levels") is not None and locals().get("spot") is not None:
            st.markdown("### Live Level Map")
            render_live_level_chart(_chart_ohlc, spot, levels, f"{selected} • Live Levels")
    except Exception:
        pass
